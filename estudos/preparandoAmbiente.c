//Compilação: gcc preparandoAmbiente.c -o AS $(pkg-config allegro-5 allegro_main-5 allegro_font-5 --libs --cflags)
...
int main(){
	...
	ALLEGRO_TIMER* timer = al_create_timer(1.0 / 30.0);       //Quantidade de fps   //Cria o relógio do jogo; isso indica quantas atualizações serão realizadas por segundo (30, neste caso)
	...
	while(1){																		//Laço principal do programa
		...
		if (event.type == 30){														//O evento tipo 30 indica um evento de relógio, ou seja, verificação se a tela deve ser atualizada (conceito de FPS)
			al_clear_to_color(al_map_rgb(40, 0, 0));									//Substitui tudo que estava desenhado na tela por um fundo preto
