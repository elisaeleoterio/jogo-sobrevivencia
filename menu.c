...
int menu(struct mundo *mundo) {
    ...
    ALLEGRO_FONT *font_menu = al_load_font("assets/fonts/LaBelleAurore-Regular.ttf", 60, 0);
    if (!font_menu) {
        al_destroy_bitmap(background);
    ...
    while (!menu_done) {
        ...
        int font_height = al_get_font_line_height(font_menu);

        // Ponto X central
        float menu_x = mundo->largura / 2.0; 
        // Ponto Y central
        float menu_y = mundo->altura / 4.0;
